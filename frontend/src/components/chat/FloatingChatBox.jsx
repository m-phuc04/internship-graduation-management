      ...
      {isWidgetOpen && (
        <div className="fixed bottom-4 right-4 sm:right-6 z-50 w-[92vw] sm:w-[380px] h-[520px] max-h-[85vh] bg-white rounded-3xl shadow-2xl border border-slate-200/90 flex flex-col overflow-hidden animate-fade-in-up">
          ...
          {currentView === 'inbox' && (
            <>
              {/* Inbox Header */}
              <div className="px-5 py-4 bg-gradient-to-r from-[#123891] to-[#1B4DA1] text-white flex items-center justify-between shrink-0 shadow-xs">
              ...
              <div className="p-3 border-b border-slate-100 bg-white">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full py-1.5 pl-8 pr-3 bg-slate-100 hover:bg-slate-100/80 focus:bg-white border border-transparent focus:border-[#123891] rounded-xl text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none transition"
                  />
                  {searchQuery && (
                    <button
                      type="button"
              ...
              <div className="flex-1 overflow-y-auto divide-y divide-slate-100 bg-white">
                {loadingConversations ? (
                  <div className="p-8 flex flex-col items-center justify-center space-y-2.5">
                    <div className="w-6 h-6 border-2 border-[#123891] border-t-transparent rounded-full animate-spin" />
                    <span className="text-xs text-slate-400 font-medium">Đang tải danh sách...</span>
                  </div>
                ) : filteredConversations.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-3">
                    <div className="w-14 h-14 rounded-2xl bg-blue-50 text-[#1B4DA1] flex items-center justify-center shadow-2xs">
                  ...
                  filteredConversations.map((conv) => {
                    ...
                    return (
                      <div
                        ...
                        className={`p-3.5 flex items-center gap-3 hover:bg-blue-50/50 transition cursor-pointer group ${
                      ...
                      >
                        ...
                        <div className="relative shrink-0">
                          {otherAvatar ? (
                            ...
                            <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-[#123891] to-[#1B4DA1] text-white font-bold text-xs flex items-center justify-center border border-white shadow-2xs">
                              {otherInitial}
                            </div>
                          )}
                        </div>
          ...
          {currentView === 'chat' && (
            <>
              {/* Chat Header */}
              <div className="px-4 py-3 bg-gradient-to-r from-[#123891] to-[#1B4DA1] text-white flex items-center justify-between shrink-0 shadow-xs select-none">
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[10px] text-blue-100/90 truncate">
              ...
              <div className="flex-1 p-4 overflow-y-auto bg-slate-50/70 space-y-3">
                {activeChat.loading ? (
                  <div className="h-full flex flex-col items-center justify-center space-y-2.5 py-12">
                    <div className="w-7 h-7 border-2 border-[#123891] border-t-transparent rounded-full animate-spin" />
                    <span className="text-xs text-slate-400 font-medium">Đang kết nối hội thoại...</span>
                  </div>
                ) : activeChat.messages.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-3">
                    <div className="w-12 h-12 rounded-2xl bg-blue-50 text-[#1B4DA1] flex items-center justify-center shadow-2xs">
                      <MessageSquare className="w-6 h-6" />
                    </div>
                    <div>
                        ...
                        >
                          <div
                            className={`p-3 rounded-2xl text-xs leading-relaxed break-words shadow-2xs ${
                              isMe
                                ? 'bg-[#123891] text-white rounded-br-xs'
              ...
              <form onSubmit={handleSend} className="p-3 bg-white border-t border-slate-100 flex items-center gap-2">
                <input
                  ...
                  className="flex-1 py-2 px-3.5 bg-slate-100 hover:bg-slate-100/80 focus:bg-white border border-transparent focus:border-[#123891] rounded-2xl text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none transition shadow-2xs"
                />

                <button
                  type="submit"
                  disabled={!inputMessage.trim()}
                  className="w-8 h-8 rounded-full bg-[#123891] hover:bg-[#1B4DA1] hover:text-[#ECA124] disabled:opacity-40 disabled:hover:bg-[#123891] text-white flex items-center justify-center shrink-0 transition shadow-xs cursor-pointer"
                  title="Gửi tin nhắn"
                >
