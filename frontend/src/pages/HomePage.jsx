...
const HomePage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f4f7fb] text-slate-800 flex flex-col font-sans selection:bg-[#123891] selection:text-white">
      {/* 1. Header Bar (White bg, blue #1B4DA1 text, gold #ECA124 hover) */}
      <header className="bg-white text-[#1B4DA1] border-b border-slate-200 sticky top-0 z-40 shadow-xs">
        <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-4">
          {/* Logo & School Name */}
          <Link to="/" className="flex items-center gap-3 group">
            <div className="p-1.5 sm:p-2 bg-white rounded-xl shadow-xs border border-slate-100 flex items-center justify-center shrink-0">
              <IUHLogo className="h-8 sm:h-9 w-auto object-contain" />
            </div>
            <div className="border-l border-slate-200 pl-3">
              <div className="text-xs sm:text-sm font-black text-[#1B4DA1] tracking-tight leading-tight group-hover:text-[#ECA124] transition">
                CỔNG THÔNG TIN TTDN & KLTN
              </div>
              <div className="text-[10px] sm:text-[11px] font-bold text-slate-500 uppercase tracking-wide mt-0.5">
          ...
          <div className="flex items-center gap-3">
            {isAuthenticated && (
              <div className="flex items-center gap-2 sm:gap-3">
                <Link
                  to={getDashboardUrl()}
                  className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-[#123891] hover:bg-[#1B4DA1] text-white hover:text-[#ECA124] text-xs font-black shadow-sm transition"
                >
                ...
                <div className="flex items-center gap-2 p-1 pl-2.5 bg-slate-50 rounded-xl border border-slate-200 text-xs text-slate-700">
                  <div className="text-right hidden sm:block">
                    <span className="font-bold text-[#1B4DA1] text-xs block leading-tight">{user?.fullName}</span>
                    <span className="text-[10px] text-[#ECA124] font-bold">{getRoleLabel(user?.role)}</span>
                  </div>
                  <button
                    type="button"
                    onClick={handleLogout}
                    title="Đăng xuất"
                    className="p-1.5 hover:bg-slate-200/60 rounded-lg text-rose-600 transition cursor-pointer"
                  >
      ...
      <main className="flex-1 w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-7">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
          ...
          <div className="lg:col-span-8 bg-white rounded-2xl sm:rounded-3xl border border-slate-200/90 shadow-xs overflow-hidden flex flex-col">
            ...
            {/* Header Title (Blue #123891 background, White text) */}
            <div className="px-5 sm:px-6 py-4 border-b border-[#0e2c73] bg-[#123891] text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-[#ECA124] text-slate-950 flex items-center justify-center shrink-0 shadow-md">
              ...
              <div className="hidden sm:flex items-center gap-1.5 text-xs text-[#ECA124] font-bold bg-[#ECA124]/15 px-3 py-1 rounded-full border border-[#ECA124]/30">
                <ShieldCheck className="w-3.5 h-3.5 text-[#ECA124]" />
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center justify-center w-13 sm:w-15 rounded-xl sm:rounded-2xl overflow-hidden border border-blue-100 shadow-2xs shrink-0 bg-white select-none">
                        <div className="w-full bg-[#123891] text-white text-[9px] sm:text-[10px] font-bold py-1 text-center uppercase tracking-wider">
                          THG {dateInfo.month}
                        </div>
                        <div className="w-full py-1 text-center text-lg sm:text-2xl font-black text-slate-800 group-hover:text-[#123891] transition">
                          {dateInfo.day}
                        </div>
                      </div>
                      ...
                      <div className="flex-1 space-y-1.5 min-w-0">
                        ...
                        <h2
                          ...
                          className="text-sm sm:text-[15px] font-bold text-slate-900 group-hover:text-[#123891] transition cursor-pointer line-clamp-2 leading-snug"
                        >
                          {item.title}
                        </h2>
            ...
            <div className="bg-slate-50/80 px-5 sm:px-6 py-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
              <span className="flex items-center gap-1.5">
                <CheckCircle2 className="w-3.5 h-3.5 text-[#123891]" />
                <span>Kênh thông báo đào tạo chính thức</span>
              </span>
              <span className="text-[#123891] font-bold">Khoa CNTT - IUH</span>
            </div>
          </div>
          ...
          <div className="lg:col-span-4 space-y-4">
            ...
            <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/90 shadow-sm p-5 sm:p-6 bg-gradient-to-b from-blue-50/30 via-white to-white relative overflow-hidden">
              ...
              <div className="text-center pb-4 border-b border-slate-100">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-50 text-[#123891] text-xs font-bold mb-1.5 border border-blue-100">
              ...
              {isAuthenticated ? (
                <div className="mt-5 space-y-4">
                  <div className="p-4 rounded-2xl bg-blue-50/60 border border-blue-100 flex items-center gap-3.5">
                    <div className="w-12 h-12 rounded-2xl bg-[#123891] text-white flex items-center justify-center font-black text-lg shrink-0 shadow-xs">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-xs font-semibold text-[#123891] mt-0.5">{getRoleLabel(user?.role)}</div>
                    </div>
                  </div>

                  <Link
                    to={getDashboardUrl()}
                    className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-[#123891] hover:bg-[#1B4DA1] text-white hover:text-[#ECA124] text-xs sm:text-sm font-bold uppercase shadow-md shadow-blue-900/20 transition cursor-pointer"
                  >
                ...
                <form onSubmit={handleLogin} className="mt-5 space-y-3.5">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        required
                        className="w-full pl-10 pr-3.5 py-2.5 bg-slate-50/70 border border-slate-200 rounded-xl text-xs sm:text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-[#123891] focus:ring-2 focus:ring-blue-200/50 transition"
                      />
                    </div>
                  </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-28 shrink-0">
                        <input
                          ...
                          required
                          autoComplete="off"
                          className="w-full px-2.5 py-2 bg-slate-50/70 border border-slate-200 rounded-xl text-xs font-bold text-center uppercase tracking-widest text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-[#123891] focus:ring-2 focus:ring-blue-200/50"
                        />
                      </div>
                      ...
                      >
                        <RotateCw className={`w-4 h-4 text-[#123891] ${captchaLoading ? 'animate-spin' : ''}`} />
                      </button>
                  ...
                  {/* Submit Button (Blue #123891, text white, hover gold #ECA124) */}
                  <div className="pt-2">
                    <button
                      type="submit"
                      disabled={submitLoading}
                      className="w-full py-3 px-4 rounded-xl text-xs sm:text-sm font-black uppercase tracking-wide text-white bg-[#123891] hover:bg-[#1B4DA1] hover:text-[#ECA124] active:scale-[0.99] shadow-md shadow-blue-900/20 transition duration-150 cursor-pointer disabled:opacity-60"
                    >
                  ...
                  <div className="pt-2 text-center border-t border-slate-100">
                    <button
                      ...
                      className="text-xs text-[#1B4DA1] hover:text-[#ECA124] font-semibold transition cursor-pointer"
                    >
                ...
                <form onSubmit={handleRegister} className="mt-5 space-y-3">
                  <div className="p-2.5 bg-blue-50 rounded-xl border border-blue-100 text-[11px] text-[#123891] font-medium flex items-center gap-1.5">
                  ...
                  <div>
                    <input
                      ...
                      maxLength={8}
                      required
                      className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-[#123891]"
                    />
                  </div>
                  ...
                  <div className="pt-1">
                    <button
                      type="submit"
                      disabled={regLoading}
                      className="w-full py-2.5 px-4 rounded-xl text-xs font-black uppercase tracking-wide text-white bg-[#123891] hover:bg-[#1B4DA1] hover:text-[#ECA124] active:scale-[0.99] shadow-md shadow-blue-900/20 transition duration-150 cursor-pointer disabled:opacity-60"
                    >
                  ...
                  <div className="text-center pt-2 border-t border-slate-100">
                    <button
                      ...
                      className="text-xs text-slate-600 hover:text-[#ECA124] font-semibold transition cursor-pointer"
                    >
                      Đã có tài khoản? <span className="underline font-bold text-[#1B4DA1] hover:text-[#ECA124]">Đăng nhập ngay</span>
                    </button>
                  </div>
                </form>
              )}
            ...
            <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/90 p-5 shadow-2xs space-y-3 text-xs">
              <div className="font-bold text-slate-800 flex items-center gap-1.5 pb-2 border-b border-slate-100">
                <Sparkles className="w-3.5 h-3.5 text-[#123891]" />
              ...
              <ul className="space-y-2 text-slate-600">
                <li>
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="hover:text-[#123891] flex items-center justify-between p-2 rounded-xl hover:bg-slate-50 transition"
                  >
